...
3. Run the address validator script: `python scripts/validate_address.py <address>` to check format and length.
4. If connecting: ask what site/app is requesting the connection. Check if it's a known phishing domain.
5. If signing: explain exactly what the signature authorizes. Warn if it's an "approve all" or unlimited token allowance.
6. If swapping: verify the token contract address on a block explorer (Etherscan, etc.).
